...
class WatchlistControllerTest {
    ...
    void setUp() {
        WatchlistController controller = new WatchlistController(
                symbolRegistry, targetPriceProvider, livePriceCache, symbolManagementService);
    ...
    void getWatchlist_returnsRowsWithExchangeLabels() throws Exception {
        ...
        when(livePriceCache.getAll()).thenReturn(Map.of(
                "AAPL", 182.0,
                "SAP.DE", 175.0,
                "005930.KS", 68000.0));
    ...
    void addSymbol_200onSuccess() throws Exception {
        ...
        mockMvc.perform(post("/api/symbols")
    ...
    void removeSymbol_200whenFound() throws Exception {
        ...
        mockMvc.perform(delete("/api/symbols/AAPL"))
                .andExpect(status().isOk());
    }
    ...
    void removeSymbol_404whenNotFound() throws Exception {
        ...
        mockMvc.perform(delete("/api/symbols/UNKNOWN"))
    ...
    void setTarget_200whenSymbolFound() throws Exception {
        ...
        verify(targetPriceProvider).updateTargetPrice(eq(aapl), eq(TargetSide.BUY), eq(170.0), eq(AssetType.STOCK));
    }
