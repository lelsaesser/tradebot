...
-   **`YahooFinanceClient`:** Fetches daily OHLCV for international stocks (German/Korean) AND intraday price quotes (via `meta.regularMarketPrice`). No auth. **Transport in flight:** legacy curl/ProcessBuilder path coexists with a new `java.net.http.HttpClient` path, gated by `FeatureToggle.YAHOO_HTTP_CLIENT` (default OFF). #435 motivated the migration based on ta4j evidence; #457 tracks verification + cleanup. 3s delay between calls. Metered via `ApiRequestMeteringService`. Failures throw `YahooFetchException` (caught silently in OhlcvFetcher and YahooPriceEvaluator — no Telegram alert).
...
-   **ProcessBuilder Shell-Out Pattern**: `YahooFinanceClient.executeCurl()` uses Java ProcessBuilder to run curl with specific headers. Originally introduced under the assumption that Yahoo blocks Java HTTP clients via TLS fingerprinting. **This justification is suspect:** ta4j hits the same Yahoo endpoint with plain `java.net.http.HttpClient` and only a `User-Agent: Mozilla/5.0` header (#435), suggesting the original block was a UA filter on `RestTemplate`'s default UA, not TLS fingerprinting. A parallel `HttpClient` path exists in `YahooFinanceClient.executeRequest()` and is selectable via `FeatureToggle.YAHOO_HTTP_CLIENT`. Verification window: 3 trading days minimum on toggle ON before #457 removes curl. Includes `--connect-timeout 5 --max-time 10` curl flags + `waitFor(15s)` Java backstop + `destroyForcibly()` on timeout.
