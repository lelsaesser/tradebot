...
-   **`TailRiskService`:** Calculates excess kurtosis and skewness from daily price changes to detect fat tail risk. Lookback is a method parameter on `analyzeTailRisk(symbol, displayName, lookbackDays)` so callers can run multiple windows in parallel. Canonical values: 35 (short, ~25 trading days) and 252 (long, ~1 year). Returns `Optional.empty()` when fewer than `lookbackDays` prices are available.
-   **`TailRiskTracker`:** Monitors sector ETFs for elevated tail risk with directional context. Runs analysis twice per scheduler tick (35-day and 252-day windows). Alerts: up to two independent Telegram messages per run, sent in stable order (35d first, 252d second), each tagged with `` `[35d]` `` / `` `[252d]` `` in the title. Daily report: single combined Telegram message with two tagged sections (`[35d window]` / `[252d window]`). Emits a `TAIL_RISK_COMPARE` INFO log line per symbol per daily run for offline grep-based analysis (issue #336 comparison period).
