...
-   **`SqliteTreasuryIndicatorStateRepository`:** Per-series state for the FRED Treasury macro indicators tracked by `TreasuryTracker`. Stores one row per FRED series (`T10Y3M`, `T10Y2Y`, `DFII10`, `THREEFYTP10`) carrying the last classification band, raw value, observation date, and an `initialized` flag (false on first write to suppress ghost-transition alerts after a fresh deploy — matches `momentum_roc_state` pattern). `INSERT OR REPLACE` on save. Methods: `save(TreasuryIndicatorState)`, `findBySeriesId(String)`. Survives bot restarts so we never silently miss a yield-curve inversion event mid-restart. Added in #516.
