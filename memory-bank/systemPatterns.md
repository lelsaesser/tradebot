...
-   **`RsiService`:** Core service for RSI calculations. Manages historical price data, calculates RSI values, detects market holidays, and accumulates overbought/oversold signals. Uses batched reporting pattern — signals collected during `addPrice()` calls are sent as a consolidated hourly report via `sendRsiReport()`. Implements delete-before-send pattern (same as BB). Integrates cached current prices for accurate on-demand RSI queries.
