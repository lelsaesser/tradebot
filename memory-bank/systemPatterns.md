...
-   **`EmaService`:** Calculates 5 EMAs (9, 21, 50, 100, 200 day) for a symbol using `StatisticsUtil.calculateEma()`. Requires 200+ data points (fetches 400 calendar days for warm-up). Classifies each symbol via `EmaSignalType.fromEmasBelow()`. Returns `Optional<EmaAnalysis>`.
